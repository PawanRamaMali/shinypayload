...
.read_request_body <- function(rook_input) {
  if (is.null(rook_input)) return(raw(0))
  ...
  if (length(body_parts) == 0) return(raw(0))
  do.call(c, body_parts)
}

.parse_request_body <- function(req, body_raw) {
  if (length(body_raw) == 0) return(NULL)
...
.check_auth <- function(req, token) {
  if (is.null(token) || token == "") return(TRUE)
  
  # Check query parameter
  query_params <- shiny::parseQueryString(req$QUERY_STRING %||% "")
  if (identical(query_params$token, token)) return(TRUE)
  
  # Check headers
  headers <- req$HEADERS %||% list()
  
  # Check X-Ingress-Token header
  if (identical(headers[["x-ingress-token"]], token)) return(TRUE)
  
  # Check Authorization header
  auth_header <- headers[["authorization"]] %||% req$HTTP_AUTHORIZATION
  if (identical(auth_header, token)) return(TRUE)
...
#' @return A function(req) that handles both regular Shiny UI and POST requests
#' @export
#' @examples
#' \dontrun{
#' ui <- payload_ui(
...
#' @return A reactive expression that returns list(payload=..., meta=...) or NULL
#' @export
#' @examples
#' \dontrun{
...
#' @return Character string with the full URL
#' @export
#' @examples
#' \dontrun{
#' server <- function(input, output, session) {
